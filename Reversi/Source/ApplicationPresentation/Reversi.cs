...
using FullSailAFI.GamePlaying.ExampleAI;
...
namespace FullSailAFI.GamePlaying
{
	...
	public class ReversiForm : System.Windows.Forms.Form
	{
        ...
        // Full Sail AI Behavior for black
        private MinimaxAFI fullSailAIBehaviorBlack = new MinimaxAFI("Black");

        // Full Sail AI Behavior for white
        private MinimaxAFI fullSailAIBehaviorWhite = new MinimaxAFI("White");
		...
		public ReversiForm()
		{
            ...
            if (options.BlackUsesFullSailAI)
                this.agents[0].behavior = fullSailAIBehaviorBlack;
            else
                this.agents[0].behavior = studentBehavior;
			this.agents[1] = new Agent();
            if (options.WhiteUsesFullSailAI)
                this.agents[1].behavior = fullSailAIBehaviorWhite;
            else
            ...
            // Hand over initial settings to Full Sail AI.

            if (this.options.BlackUsesFullSailAI)
            {
                fullSailAIBehaviorBlack.fullSailAiMode              = this.options.BlackFullSailAiMode;
                fullSailAIBehaviorBlack.isTranspositionTableOn      = this.options.BlackTranspositionTable;
                fullSailAIBehaviorBlack.mctsModeSimulationNumber    = this.options.BlackMctsSimulationNumber;
                fullSailAIBehaviorBlack.cachingMemoryNumber         = this.options.BlackCachingMemoryNumber;
            }
            if (this.options.WhiteUsesFullSailAI)
            {
                fullSailAIBehaviorWhite.fullSailAiMode              = this.options.WhiteFullSailAiMode;
                fullSailAIBehaviorWhite.isTranspositionTableOn      = this.options.WhiteTranspositionTable;
                fullSailAIBehaviorWhite.mctsModeSimulationNumber    = this.options.WhiteMctsSimulationNumber;
                fullSailAIBehaviorWhite.cachingMemoryNumber         = this.options.WhiteCachingMemoryNumber;
            }

            // A bit hacky, but bring window to front (in case visualization is running).
            this.Show();
            this.Activate();
            this.TopMost = true;
            this.TopMost = false;
		...
		private void InitializeComponent()
		{
            ...
            this.blackTextLabel.Size = new System.Drawing.Size(61, 20);
		...
		private void LoadProgramSettings()
		{
			...
			try
			{
                ...
                this.options.BlackFullSailAiMode        = (int)  System.Int32.Parse(this.settings.GetValue("Options", "BlackFullSailAiMode"));
                this.options.BlackTranspositionTable    = (bool) System.Boolean.Parse(this.settings.GetValue("Options", "BlackTranspositionTable"));
                this.options.BlackMctsSimulationNumber  = (int)  System.Int32.Parse(this.settings.GetValue("Options", "BlackMctsSimulationNumber"));
                this.options.BlackCachingMemoryNumber   = (int)  System.Int32.Parse(this.settings.GetValue("Options", "BlackCachingMemoryNumber"));
                this.options.WhiteFullSailAiMode        = (int)  System.Int32.Parse(this.settings.GetValue("Options", "WhiteFullSailAiMode"));
                this.options.WhiteTranspositionTable    = (bool) System.Boolean.Parse(this.settings.GetValue("Options", "WhiteTranspositionTable"));
                this.options.WhiteMctsSimulationNumber  = (int)  System.Int32.Parse(this.settings.GetValue("Options", "WhiteMctsSimulationNumber"));
                this.options.WhiteCachingMemoryNumber   = (int)  System.Int32.Parse(this.settings.GetValue("Options", "WhiteCachingMemoryNumber"));
		...
		private void SaveProgramSettings()
		{
            ...
            this.settings.SetValue("Options", "BlackFullSailAiMode", this.options.BlackFullSailAiMode.ToString());
            this.settings.SetValue("Options", "BlackTranspositionTable", this.options.BlackTranspositionTable.ToString());
            this.settings.SetValue("Options", "BlackMctsSimulationNumber", this.options.BlackMctsSimulationNumber.ToString());
            this.settings.SetValue("Options", "BlackCachingMemoryNumber", this.options.BlackCachingMemoryNumber.ToString());
            this.settings.SetValue("Options", "WhiteFullSailAiMode", this.options.WhiteFullSailAiMode.ToString());
            this.settings.SetValue("Options", "WhiteTranspositionTable", this.options.WhiteTranspositionTable.ToString());
            this.settings.SetValue("Options", "WhiteMctsSimulationNumber", this.options.WhiteMctsSimulationNumber.ToString());
            this.settings.SetValue("Options", "WhiteCachingMemoryNumber", this.options.WhiteCachingMemoryNumber.ToString());
		...
		private void optionsMenuItem_Click(object sender, System.EventArgs e)
		{
			...
			if (dlg.ShowDialog(this) == DialogResult.OK)
			{
                ...
                this.options.BlackUsesFullSailAI    = dlg.Options.BlackUsesFullSailAI;
                this.options.WhiteUsesFullSailAI    = dlg.Options.WhiteUsesFullSailAI;
                if (this.options.BlackUsesFullSailAI)
                    this.agents[0].behavior = fullSailAIBehaviorBlack;
                else
                    this.agents[0].behavior = studentBehavior;
                if (this.options.WhiteUsesFullSailAI)
                    ...
                    this.agents[1].behavior = studentBehavior;
                this.options.BlackFullSailAiMode                    = dlg.Options.BlackFullSailAiMode;
                this.options.BlackTranspositionTable                = dlg.Options.BlackTranspositionTable;
                this.options.BlackMctsSimulationNumber              = dlg.Options.BlackMctsSimulationNumber;
                this.options.BlackCachingMemoryNumber               = dlg.Options.BlackCachingMemoryNumber;
                this.options.WhiteFullSailAiMode                    = dlg.Options.WhiteFullSailAiMode;
                this.options.WhiteTranspositionTable                = dlg.Options.WhiteTranspositionTable;
                this.options.WhiteMctsSimulationNumber              = dlg.Options.WhiteMctsSimulationNumber;
                this.options.WhiteCachingMemoryNumber               = dlg.Options.WhiteCachingMemoryNumber;
                if (this.options.BlackUsesFullSailAI)
                {
                    fullSailAIBehaviorBlack.fullSailAiMode              = dlg.Options.BlackFullSailAiMode;
                    fullSailAIBehaviorBlack.isTranspositionTableOn      = dlg.Options.BlackTranspositionTable;
                    fullSailAIBehaviorBlack.mctsModeSimulationNumber    = dlg.Options.BlackMctsSimulationNumber;
                    fullSailAIBehaviorBlack.cachingMemoryNumber         = dlg.Options.BlackCachingMemoryNumber;
                }
                if (this.options.WhiteUsesFullSailAI)
                {
                    fullSailAIBehaviorWhite.fullSailAiMode              = dlg.Options.WhiteFullSailAiMode;
                    fullSailAIBehaviorWhite.isTranspositionTableOn      = dlg.Options.WhiteTranspositionTable;
                    fullSailAIBehaviorWhite.mctsModeSimulationNumber    = dlg.Options.WhiteMctsSimulationNumber;
                    fullSailAIBehaviorWhite.cachingMemoryNumber         = dlg.Options.WhiteCachingMemoryNumber;
                }
